...
def GA(p_max): # Define function to process 
...
def GA_RA(p_max): # Define function to process 
    print(f"calculaiton for p_max for GA_RA",p_max)
    all_best_combinations = []
    sum_fitness_current = 0 # Initialize sum of fitness
...
def HC(p_max):
    print(f"calculaiton for p_max for HC_RA", p_max)
    all_best_combinations = []
    sum_fitness_current = 0  # Initialize sum of fitness
...
# Define a function to process
def RS(p_max):
    print(f"calculaiton for p_max for RS",p_max) 
    all_best_combinations = []
    sum_fitness_current = 0 # Initialize sum of fitness
...
if __name__ == '__main__': # Add this to prevent issues in multiprocessing on Windows
    Power_values = np.arange(1, 11,1) # Power values from 1 to 10
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        fitness_sums_GA = pool.map(GA, Power_values)
        fitness_sums_GA_RA = pool.map(GA_RA, Power_values)
        fitness_sums_HC = pool.map(HC, Power_values)
        fitness_sums_HC_RA = pool.map(HC_RA, Power_values)
        fitness_sums_RS = pool.map(RS, Power_values)

    data_dict = {
        "Generation": Power_values, 
    ...
    csv_file_path_pandas = "fitness_summary Power.csv"

    # Create a Pandas DataFrame from the dictionary
    df = pd.DataFrame(data_dict)

    # Your data arrays (replace with your actual data)
    Power_values = np.arange(1, 11, 1)
    ...
    # Plot the data
    plt.plot(Power_values, fitness_sums_GA, marker='*', linestyle='dotted', label="C2GA")
    plt.plot(Power_values, fitness_sums_GA_RA, marker='s', linestyle='dotted', label="C2GA_RA")
    plt.plot(Power_values, fitness_sums_HC, marker='s', linestyle='-', label="HC")
    plt.plot(Power_values, fitness_sums_HC_RA, marker='*', linestyle='-', label="HC_RA")
    plt.plot(Power_values, fitness_sums_RS, marker='o', linestyle='dashdot', label="RS")
    ...
    # Save the figure
    plt.savefig("Energy vs Power.pdf", format="pdf", bbox_inches="tight", dpi=800)
