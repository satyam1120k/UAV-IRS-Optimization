...
def GA(Dm): # Define function to process 
    print(f"calculaiton for Dm for GA",Dm)
...
def GA_RA(Dm): # Define function to process 
    print(f"calculaiton for Dm for GA_RA",Dm)
    all_best_combinations = []
    sum_fitness_current = 0 # Initialize sum of fitness
...
def HC(Dm):
    print(f"calculaiton for Dm for HC_RA", Dm)
    all_best_combinations = []
    sum_fitness_current = 0  # Initialize sum of fitness
...
# Define a function to process
def RS(Dm):
    print(f"calculaiton for Dm for RS",Dm) 
    all_best_combinations = []
    sum_fitness_current = 0 # Initialize sum of fitness
...
if __name__ == '__main__': # Add this to prevent issues in multiprocessing on Windows
    Dm_value = np.arange(0.1,1.1,0.1) # Data size Mb from 1 to 10
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        fitness_sums_GA = pool.map(GA, Dm_value)
        fitness_sums_GA_RA = pool.map(GA_RA, Dm_value)
        fitness_sums_HC = pool.map(HC, Dm_value)
        fitness_sums_HC_RA = pool.map(HC_RA, Dm_value)
        fitness_sums_RS = pool.map(RS, Dm_value)
    ...
    csv_file_path_pandas = "fitness_summary Data Size.csv"
    ...
    Dm_value = np.arange(0.1, 1.1, 0.1)
    ...
    # Plot the data
    plt.plot(Dm_value, fitness_sums_GA, marker='>', linestyle='dotted', label="C2GA")
    plt.plot(Dm_value, fitness_sums_GA_RA, marker='v', linestyle='dotted', label="C2GA_RA")
    plt.plot(Dm_value, fitness_sums_HC, marker='s', linestyle='-', label="HC")
    plt.plot(Dm_value, fitness_sums_HC_RA, marker='*', linestyle='-', label="HC_RA")
    plt.plot(Dm_value, fitness_sums_RS, marker='o', linestyle='dashdot', label="RS")
