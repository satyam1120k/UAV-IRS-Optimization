...
# Define keys that should be subjected to perturbation 
...
def hc_ra(hc_ra_queue):
    fitness_sums_HC = [] # Store sum of fitness 
            ...
            for j in range(num_generation): # Hill Climbing iterations
                ...
                for i in range(4):
                    ...
                    def compute_fitness(data):
                        ...
                        V_down_value = data['V_down_value'] # Retrieve angle row from neighbor data
                        V_up_value = data['V_up_value'] # Retrieve angle row from neighbor data
                        V_har_value = data['V_har_value'] # Retrieve angle row from neighbor data
    ...
    sum_fitness_current = 0 # Sum of best fitness
    for assignment in best_assignments:
        ...
        print("-" * 20)
        sum_fitness_current += best_ind['fitness'] # Sum fitness values
    ...
    fitness_sums_HC.append(sum_fitness_current) # Store sum of fitness
    ...
    sum_fitness_per_generation_HC = [0] * num_generation
    for gen_idx in range(num_generation):
        generation_sum = 0
        for combination in all_best_combinations:
            generation_sum += combination['generation_fitness'][gen_idx]
        sum_fitness_per_generation_HC[gen_idx] = generation_sum

    generation_indices = list(range(1, num_generation + 1))

    plt.figure(figsize=(10, 6))
    plt.rcParams["font.size"] = "20"
    plt.plot(generation_indices, sum_fitness_per_generation_HC, marker='o', linestyle='-')
    plt.title('HC_RA Graph')
    plt.xlabel('Generation')
    plt.ylabel('Energy')
    plt.show()

    hc_ra_queue.put(sum_fitness_per_generation_HC)
...
def hc(hc_queue):
    ...
    for l in range(num_bs):
        ...
        for k in range(num_uav_irs):
            ...
            V_down_value = V_down.iloc[i, :]
            h_l_m_row = h_l_m.iloc[k, :] # Use BS-specific h_l_m_df
            h_l_km_row = h_l_km_df_bs.iloc[i, :] # Use BS-specific h_l_km_df
            V_up_value = V_up.iloc[i, :] 
            g_l_m_row = g_l_m.iloc[k, :] # Use BS-specific g_l_m_df
            g_l_km_row = g_l_km_df_bs.iloc[i, :] # Use BS-specific g_l_km_df
            V_har_value = V_har.iloc[i, :] 
            f_l_m_row = f_l_m.iloc[k, :] # Use BS-specific f_l_m_df
            ...
            for j in range(num_generation): # Hill Climbing iterations
                ...
                for i in range(4): # You can keep this loop if it's intended for repeated perturbations per generation
                    ...
                    def compute_fitness(data):
                        ...
                        V_down_value = data['V_down_value'] # Retrieve angle row from neighbor data
                        V_up_value = data['V_up_value'] # Retrieve angle row from neighbor data
                        V_har_value = data['V_har_value'] # Retrieve angle row from neighbor dataa
    ...
    hc_queue.put(sum_fitness_per_generation_HC)

def ga_ra(ga_ra_queue):
    fitness_sums_GA= [] # Store sum of fitness values
    all_best_combinations = []
                ...
                for x in range(0,S, 2): # Loop through population with step of 2
                    ...
                    def compute_fitness(data):
                        ...
                        current_data = {
                            ...
                            'V_down_value':V_down_value,
                            'V_up_value':V_up_value,
                            'V_har_value': V_har_value,# Carry forward original index# Carry forward original index
                        }
                        if V_lm_hfly_value>0 and T_m_har_value>0 and T_ml_down_value>0 and T_km_up_value>0 and P_m_har_value<=p_max and P_m_down_value<=p_max and P_km_up_value<=p_km_max and (T_km_com_value+T_km_up_value+T_ml_down_value)<=T_m and f_km_value>0  and E_kml_har_value>=(E_kml_up_value+E_kml_com_value) and V_lm_vfly_value>0:
                            return fitness_value, current_data
                        else:
    ...
    fitness_sums_GA.append(sum_fitness_current) # Store sum of 
    ...
    sum_fitness_per_generation_GA_RA = [0] * num_generation
    for gen_idx in range(num_generation):
        generation_sum = 0
        for combination in all_best_combinations:
            generation_sum += combination['generation_fitness'][gen_idx]
        sum_fitness_per_generation_GA_RA[gen_idx] = generation_sum

    generation_indices = list(range(1, num_generation + 1))

    plt.figure(figsize=(10, 6))
    plt.rcParams["font.size"] = "20"
    plt.plot(generation_indices, sum_fitness_per_generation_GA_RA, marker='o', linestyle='-')
    plt.title('GA_RA Graph')
    plt.xlabel('Generation')
    plt.ylabel('Energy')
    plt.show()
    
    ga_ra_queue.put(sum_fitness_per_generation_GA_RA)

def ga(ga_queue):
    fitness_sums_GA= [] # Store sum of fitness 
    all_best_combinations = []
    all_best_individuals = []
        # Main Genetic Algorithm Loop
    for l in range(num_bs): # for eac base station
        all_best_individuals_bs = []
        P_m_har_value = P_m_har.values[l]
        T_m_har_value = T_m_har.values[l]
        P_m_down_value = P_m_down.values[l]
        ...
        P_km_up_bs = P_km_up.iloc[valid_indices].reset_index(drop=True)
        E_ml_har_value = P_m_har_value * T_m_har_value
        for k in range(num_uav_irs): #for each UAV
            ...
            # Initialize population
            for i in range(S): # Intialize population
                f_km_value = f_km_bs[random.randint(0,population_size)] # Use BS-specific f_km
                P_km_up_value = P_km_up_bs[random.randint(0,population_size)] # Use BS-specific P_km_up
            ...
            generations_data = []
            for j in range(num_generation): #for each generaion 
                child_population = []
                for x in range(0,S, 2): # compute the child population 
                    if x + 1 >=len(valid_indices): # Check if i+1 is within bounds, if not break to avoid error in accessing population[i+1]
                        break
                    # Crossover
                    ranodmpopulation=[]
    ...
    fitness_sums_GA.append(sum_fitness_current) # Store sum of fitness 
...
def rs(rs_queue):
    fitness_sums_RA = [] # Store sum of fitness 
    all_best_combinations = []
    for l in range(num_bs):
        all_best_individuals_bs = []
        P_m_har_value = P_m_har.values[l]
    ...
    for assignment in best_assignments:
        ...
        print(f" Best Individual:")
        print(f"  Generation: {best_ind['generation']}, Type: {best_ind['type']}")
        print(f"  Fitness: {best_ind['fitness']:.4f}") # current best fitness only
        unique_indices_to_print = assignment['unique_row_indices'] # Retrieve unique_row_indices
        for key, value in best_ind['data'].items():
            if isinstance(value, pd.Series):
                print(f"  {key}: Series: \n{value.iloc[unique_indices_to_print]}") # Print sliced Series
    ...
    fitness_sums_RA.append(sum_fitness_current) # Store sum of fitness 
    ...
    sum_fitness_per_generation_RS = [0] * num_generation
    for gen_idx in range(num_generation):
        generation_sum = 0
        for combination in all_best_combinations:
            generation_sum += combination['generation_fitness'][gen_idx]
        sum_fitness_per_generation_RS[gen_idx] = generation_sum

    generation_indices = list(range(1, num_generation + 1))
    plt.figure(figsize=(10, 6))
    plt.rcParams["font.size"] = "20"
    plt.plot(generation_indices, sum_fitness_per_generation_RS, marker='o', linestyle='-')
    plt.title('RS Graph')
    plt.xlabel('Generation')
    plt.ylabel('Energy')
    plt.show()
    rs_queue.put(sum_fitness_per_generation_RS)

if __name__ == "__main__":
    hc_ra_queue = multiprocessing.Queue()  # Create a queue for hill_climbing
    hc_queue = multiprocessing.Queue()  # Create a queue for hill_climbing
    ga_ra_queue = multiprocessing.Queue()  # Create a queue for genetic_algorithm
    ...
    p1 = multiprocessing.Process(target=hc_ra, args=(hc_ra_queue,))
    p2 = multiprocessing.Process(target=hc, args=(hc_queue,))
    p3 = multiprocessing.Process(target=ga_ra, args=(ga_ra_queue,))
    p4 = multiprocessing.Process(target=ga, args=(ga_queue,))
    ...
    sum_fitness_per_generation_hc_ra = hc_ra_queue.get()
    sum_fitness_per_generation_hc = hc_queue.get()
    sum_fitness_per_generation_ga_ra = ga_ra_queue.get()
    sum_fitness_per_generation_ga = ga_queue.get()
    sum_fitness_per_generation_rs  = rs_queue.get()

    data_dict = {
        "Generation": list(range(1, len(sum_fitness_per_generation_hc_ra) + 1)) if sum_fitness_per_generation_hc_ra else [], # Assuming generation number starts from 1
        "Fitness_Sum_HC_RA": sum_fitness_per_generation_hc_ra,
        "Fitness_Sum_HC": sum_fitness_per_generation_hc,
        "Fitness_Sum_GA_RA": sum_fitness_per_generation_ga_ra,
        "Fitness_Sum_GA": sum_fitness_per_generation_ga,
        "Fitness_Sum_RS": sum_fitness_per_generation_rs,
    }
    csv_file_path_pandas = "fitness_summary_Generation.csv"
    ...
    # Plot the data
    plt.plot(generation_indices, sum_fitness_per_generation_hc_ra, label="HC_RA", marker='*', linestyle='-')
    plt.plot(generation_indices, sum_fitness_per_generation_hc, marker='s', linestyle='-', label="HC")
    plt.plot(generation_indices, sum_fitness_per_generation_ga_ra, label="C2GA_RA", marker='*', linestyle='dotted')
    plt.plot(generation_indices, sum_fitness_per_generation_ga, marker='s', linestyle='dotted', label="C2GA")
    ...
    # Save the figure
    plt.savefig("Energy vs Generation.pdf", format="pdf", bbox_inches="tight", dpi=800)

    # Show the plot
    plt.show()

    min_energy_GA = sum(sum_fitness_per_generation_ga)
    min_energy_GA_RA = sum(sum_fitness_per_generation_ga_ra)
    min_energy_HC = sum(sum_fitness_per_generation_hc)
    min_energy_HC_RA = sum(sum_fitness_per_generation_hc_ra)
    min_energy_RS = sum(sum_fitness_per_generation_rs)

    # Calculate percentage improvement over Random Search
    improvement_GA_vs_GA_RA = ((min_energy_GA_RA - min_energy_GA) / min_energy_GA_RA) * 100
    improvement_GA_vs_HC  = ((min_energy_HC - min_energy_GA) / min_energy_HC) * 100
    improvement_GA_vs_HC_RA  = ((min_energy_HC_RA - min_energy_GA) / min_energy_HC_RA) * 100
    improvement_GA_vs_RS  = ((min_energy_RS - min_energy_GA) / min_energy_RS) * 100
    ...
    print(f"Improvement of GA_IRS vs GA_IRS_RA: {improvement_GA_vs_GA_RA:.2f}%")
    print(f"Improvement of GA_IRS vs HC_IRS: {improvement_GA_vs_HC:.2f}%")
    print(f"Improvement of GA_IRS vs HC_IRS_RA: {improvement_GA_vs_HC_RA:.2f}%")
    print(f"Improvement of GA_IRS vs RS: {improvement_GA_vs_RS:.2f}%")