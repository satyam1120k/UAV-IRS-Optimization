...
V_down=pd.read_csv(r'Dataset\V_lm_down.csv') # number of IRS is 500 store in each column
h_l_km=pd.read_csv(r'Dataset\h_l_km.csv') # number of IRS is 500 store in each column
h_l_m=pd.read_csv(r'Dataset\h_l_m.csv') # number of IRS is 500 store in each column

V_up=pd.read_csv(r'Dataset\V_lm_up.csv') # number of IRS is 500 store in each column
g_l_km=pd.read_csv(r'Dataset\g_l_km.csv') # number of IRS is 500 store in each column
g_l_m=pd.read_csv(r'Dataset\g_l_m.csv') # number of IRS is 500 store in each column # corrected filename

V_har=pd.read_csv(r'Dataset\V_lm_har.csv') # number of IRS is 500 store in each column
f_l_km=pd.read_csv(r'Dataset\f_l_km.csv') # number of IRS is 500 store in each column
f_l_m=pd.read_csv(r'Dataset\f_l_m.csv') # number of IRS is 500 store in each column # corrected filename
...
def GA(Wl_value): # Define function to process 
...
def GA_RA(Wl_value): # Define function to process 
    print(f"calculaiton for Wl_value for GA_RA",Wl_value)
    all_best_combinations = []
    sum_fitness_current = 0 # Initialize sum of fitness
...
def HC(Wl_value):
    print(f"calculaiton for Wl_value for HC_RA", Wl_value)
    all_best_combinations = []
    sum_fitness_current = 0  # Initialize sum of fitness
...
# Define a function to process
def RS(Wl_value):
    print(f"calculaiton for Wl_value for RS",Wl_value) 
    all_best_combinations = []
    sum_fitness_current = 0 # Initialize sum of fitness
...
if __name__ == '__main__': # Add this to prevent issues in multiprocessing on Windows
    Wl_value = np.arange(10, 51,5) # Power values from 1 to 10
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        fitness_sums_GA = pool.map(GA, Wl_value)
        fitness_sums_GA_RA = pool.map(GA_RA, Wl_value)
        fitness_sums_HC = pool.map(HC, Wl_value)
        fitness_sums_HC_RA = pool.map(HC_RA, Wl_value)
        fitness_sums_RS = pool.map(RS, Wl_value)
    ...
    csv_file_path_pandas = "fitness_summary Weight.csv"

    # Create a Pandas DataFrame from the dictionary
    df = pd.DataFrame(data_dict)

    # Your data arrays (replace with your actual data)
    Wl_value = np.arange(10, 51, 5)

    # Create the plot
    plt.figure(figsize=(12, 7))

    # Set font parameters
    plt.rcParams["font.size"] = "26"  # Reduced from 30 to ensure labels fit
    plt.rcParams["font.family"] = "Times New Roman"

    # Plot the data
    plt.plot(Wl_value, fitness_sums_GA, marker='*', linestyle='dotted', label="C2GA")
    plt.plot(Wl_value, fitness_sums_GA_RA, marker='s', linestyle='dotted', label="C2GA_RA")
    plt.plot(Wl_value, fitness_sums_HC, marker='s', linestyle='-', label="HC")
    plt.plot(Wl_value, fitness_sums_HC_RA, marker='*', linestyle='-', label="HC_RA")
    plt.plot(Wl_value, fitness_sums_RS, marker='o', linestyle='dashdot', label="RS")

    # Set axis labels
    plt.xlabel("Weights of UAV")
    plt.ylabel('Energy')
    ...
    # Save the figure
    plt.savefig("Energy vs Weight.pdf", format="pdf", bbox_inches="tight", dpi=800)
